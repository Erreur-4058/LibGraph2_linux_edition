...
Ce rapport détaille les étapes nécessaires pour porter la bibliothèque LibGraph2 de Windows vers Linux en remplaçant les API Windows (Win32, GDI+) par des bibliothèques multiplateformes (SFML et tinyfiledialogs).

### Technologies Actuelles (Windows)
- **Fenêtrage** : Win32 API (`CreateWindow`, `GetMessage`, `DispatchMessage`)
- **Rendu 2D** : GDI+ (`Graphics`, `Pen`, `Brush`, `Image`)
- **Boîtes de dialogue** : API Windows natives (`ChooseColor`, `IFileDialog`, `MessageBox`)
- **Build** : Visual Studio Project Files (`.vcxproj`)

### Technologies Cibles (Linux)
- **Fenêtrage et Rendu** : SFML (`sf::RenderWindow`, `sf::Shape`, `sf::Sprite`)
- **Boîtes de dialogue** : tinyfiledialogs
- **Build** : CMake
...
- `LibGraph2.vcxproj`
- `LibGraph2.vcxproj.filters`
- `LibGraph2.vcxproj.user`

### Fichier à Créer : CMakeLists.txt
...
# Sources principales (après refactoring)
set(SOURCES
    LibGraph2.cpp
    LibGraph2impSFML.cpp
    # Ajouter autres fichiers .cpp nécessaires
)
...
## Phase 2 : Refactoring de l'Architecture de Fenêtrage

### Fichiers Concernés
- `LibGraphWnd.h/.cpp` ❌ **À SUPPRIMER**
- `LibGraphMainWnd.h/.cpp` ❌ **À SUPPRIMER**
- `LibGraphGraphicWnd.h/.cpp` ❌ **À SUPPRIMER**
- `LibGraphConsoleWnd.h/.cpp` ❌ **À SUPPRIMER**
- `CaptionBar.h/.cpp` ❌ **À SUPPRIMER**
- `LibGraphCinStreamBuf.h/.cpp` ❌ **À SUPPRIMER**

### Changements Architecturaux

#### AVANT (Windows - Hiérarchie complexe)
```cpp
// LibGraphWnd.h - Classe de base pour fenêtres Win32
class CLibGraphWnd {
private:
    HWND m_hWnd;
...
// LibGraphMainWnd.h - Fenêtre principale
class CLibGraphMainWnd : public CLibGraphWnd {
private:
    CLibGraphGraphicWnd m_wndGraphicWnd;  // Fenêtre de dessin
    CLibGraphConsoleWnd m_wndConsoleWnd;  // Fenêtre console
...
#### APRÈS (Linux - Architecture simplifiée)
```cpp
// LibGraph2.h - Classe unique encapsulant SFML
class CLibGraph2 {
private:
    sf::RenderWindow* m_pWindow;
    // Plus besoin de hiérarchie de fenêtres
...
**Problème Windows** : Sur Windows, LibGraph2 créait une console personnalisée intégrée à la fenêtre avec redirection de `stdout`/`stderr` via des pipes.

**Solution Linux** : Sur Linux, `std::cout` et `std::cin` fonctionnent nativement dans le terminal. Toute la logique de `LibGraphConsoleWnd`, `LibGraphCinStreamBuf`, et la redirection de pipes dans `LibGraphMainWnd::RedirectConsoleIO()` doit être **supprimée**.

---

## Phase 3 : Remplacement du Moteur de Rendu (GDI+ → SFML)

### Fichiers Concernés
- `LibGraph2impGDIPLUS.h/.cpp` ✏️ **À REMPLACER** par `LibGraph2impSFML.h/.cpp`
- `LibGraph2impDX.h/.cpp` ❌ **À SUPPRIMER** (DirectX)
- `LibGraph2impGL.h/.cpp` ❌ **À SUPPRIMER** (OpenGL)
- `LibGraph2impCommon.cpp` ❌ **À SUPPRIMER**
- `libgraph2imp.h` ✏️ **À ADAPTER**
...
public:
    void setPen(ARGB color, float fWidth, pen_DashStyles style) {
        ...
        // Il faudra les implémenter manuellement si nécessaire
    }
...
void CLibGraph2::drawEllipse(const CRectangle& bounds) {
    ...
    // SFML utilise CircleShape avec setScale pour créer une ellipse
...
#### 5. drawPie (⚠️ DÉFI MAJEUR)

**Problème** : SFML ne fournit pas de primitive "camembert" (pie chart). Il faut la créer manuellement.
...
void CLibGraph2::drawPie(const CRectangle& bounds, float startAngle, float sweepAngle) {
    ...
    // Nécessite de dessiner les lignes du contour séparément
}
```
...
public:
    void setFont(const CString& strFontName, float fPointSize, font_styles nStyleFlags) {
        // Charger la police depuis le système
...
## Phase 4 : Remplacement des Boîtes de Dialogue
...
- `DlgInput.h/.cpp` ❌
- `DlgPenStyle.h/.cpp` ❌
- `LibGraph2.rc` ❌ (fichier de ressources Windows)
- `resource.h` ❌

### Installation de tinyfiledialogs

Télécharger `tinyfiledialogs.h` et `tinyfiledialogs.c` depuis :
https://sourceforge.net/projects/tinyfiledialogs/

### Implémentation des Fonctions GUI
...
bool CLibGraph2::guiGetFileName(CString& sFileName, bool bOpen, 
    ...
    if(bOpen) {
        result = tinyfd_openFileDialog(
            ...
            sFileName.c_str(),             // Chemin par défaut
...
bool CLibGraph2::guiGetColor(ARGB& color) {
    ...
    const char* result = tinyfd_colorChooser(
        ...
        rgbOut                   // Couleur RGB résultante
    );
    
    if(result) {
        // tinyfiledialogs ne gère pas l'alpha, conserver l'alpha existant
...
bool CLibGraph2::guiGetValue(CString& strVal, const CString& strTitle, 
    ...
    const char* result = tinyfd_inputBox(
        strTitle.c_str(),
        strLabel.c_str(),
        strVal.c_str()          // Valeur par défaut
    );
...
**Problème** : tinyfiledialogs ne fournit pas de sélecteur de police ou de style de stylo.

**Solution** : Décomposer en plusieurs boîtes de dialogue simples :
...
bool CLibGraph2::guiGetFont(CString& strFontName, float& fSize, 
    ...
    // Cette approche est limitée, à améliorer
...
bool CLibGraph2::guiGetPenStyle(ARGB& color, float& fWidth, 
    ...
    if(!guiGetValue(strWidth, "Stylo", "Épaisseur :"))
        return false;
    ...
    // Le style de trait devra être simplifié ou géré différemment
    return true;
}
```
...
- `LibGraph2impDX.h/.cpp` ❌ (DirectX)
- `LibGraph2impGL.h/.cpp` ❌ (OpenGL)
- `LibGraph2impCommon.cpp` ❌
...
**Décision** : Simplifier ou supprimer la gestion des niveaux d'expérience (0-4). Cette fonctionnalité complexifie le code et n'est pas essentielle au portage. Si conservée, adapter les exemples de documentation pour utiliser `main()` au lieu de `_tWinMain()`.
...
1. **drawPie** : SFML ne fournit pas de primitive "camembert". Nécessite une implémentation manuelle avec `sf::VertexArray` et `sf::TriangleFan`.

2. **Styles de trait pointillés** : SFML ne supporte pas nativement les styles `Dash`, `Dot`, `DashDot`. Il faudra soit :
   - Les implémenter manuellement
   - Ignorer cette fonctionnalité
   - Utiliser une bibliothèque tierce

3. **Sélecteur de police/stylo** : tinyfiledialogs ne fournit pas ces dialogues. Solutions :
   - Décomposer en plusieurs boîtes de dialogue simples
   - Créer des dialogues personnalisés avec SFML
   - Accepter une fonctionnalité réduite

### 🟡 Défis Moyens

4. **Gestion des polices** : Les chemins de polices diffèrent entre Windows et Linux. Nécessite une logique de recherche de polices système.

5. **Textures et cache d'images** : Remplacer `Gdiplus::CachedBitmap` par `sf::Texture` avec un système de cache similaire.

6. **Coordonnées normalisées** : La logique de `MapCoordinate` / `UnmapCoordinate` doit être adaptée au système de vues de SFML (`sf::View`).

### 🟢 Défis Mineurs

7. **Types de données** : Remplacer les types Windows (`DWORD`, `HWND`, etc.) par des types standards C++.

8. **Encodage de caractères** : Gérer la conversion entre `wstring` (Windows) et `string` UTF-8 (Linux).
...
| Phase 2 : Fenêtrage | Moyenne | 8-12 heures |
| Phase 3 : Rendu | Élevée | 20-30 heures |
...
1. **Approche incrémentale** : Porter une fonctionnalité à la fois et tester immédiatement.

2. **Tests** : Créer des programmes de test pour chaque fonction de dessin.

3. **Documentation** : Mettre à jour la documentation pour refléter les changements d'API.

4. **Compatibilité** : Envisager de maintenir une version Windows et une version Linux avec des `#ifdef` si nécessaire.

5. **Alternatives à drawPie** : Si l'implémentation manuelle est trop complexe, envisager d'utiliser une bibliothèque de rendu 2D plus complète comme Cairo ou Skia.

---

## Conclusion

Le portage de LibGraph2 vers Linux est **techniquement faisable** mais nécessite un **refactoring significatif**. Les principales difficultés résident dans :
- L'implémentation manuelle de certaines primitives graphiques (drawPie)
- Le remplacement des boîtes de dialogue natives
- La simplification de l'architecture de fenêtrage

SFML est un excellent choix pour ce portage car il offre une API moderne, orientée objet et performante, tout en restant relativement simple à utiliser. La combinaison SFML + tinyfiledialogs permet de couvrir la majorité des fonctionnalités de LibGraph2 avec un code portable et maintenable.
